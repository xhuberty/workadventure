...
export class SocketManager implements ZoneEventListener {
    ...
    async handleJoinRoom(client: ExSocketInterface): Promise<void> {
        ...
        try {
            ...
            let firstReadable = true;
            streamToPusher.on('readable', () => {
                // Wait for socket to be readable to send viewport info.
                // Otherwise, the handleViewport call might arrive BEFORE the joinRoomMessage message, causing issues.
                if (firstReadable) {
                    console.log('Calling handleViewport')
                    firstReadable = false;
